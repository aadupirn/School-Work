...
object Assign1 {
    def main(args: Array[String])
    {
        ...
        def findNearestPoints(testVectors: Array[Array[Float]], clusters: Array[Array[Float]]): Array[Int] = {
            testVectors.map {
        ...
        val centers = vectors.takeSample(false, K, System.nanoTime.toInt).map(r=> r._2)

        val converged = false
        val vectorArray = vectors.map(r=>r._2).collect
        var clusterAssignments = findNearestPoints(vectorArray, centers)
        ...
        while(!converged && iterations < MAX)
        {
            ...
            clusterAssignments = findNearestPoints(vectorArray, centers)
            converged = true
        ...
        println("Clusters:" + newClusters + centers)
